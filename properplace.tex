...
Like any interpretative or hermeneutic claim, our proposed typology is tentative.\end{changebar}
...
The requirement, for decision analysts to justify themselves \begin{changebar}(or to limit themselves to use assertions that are ``warranted'')\end{changebar}, is hence often taken as a self-evident, unquestionable premise. 
...
\begin{changebar}The need to address this question is illustrated by \citet{jackson_towards_1999}, who spells out in detail the resources that can be used to articulate justification, but does not explore how to ensure that an argument based on his guidelines qualifies as a justification.\end{changebar}
...
\begin{changebar}We emphasize that this task is more difficult than one might think at first sight.
...
Unfolding [i] is therefore a difficult task that can prove useful for the analyst her/himself to clarify her/his own assumptions, and one cannot assume that the exercise will always be successful.\end{changebar}
...
\begin{changebar}In some situations, this can imply enlarging the circle of people, groups or institutions involved in the decision process, for examples as members of a steering committee.\commentOC{Missing end blue}
...
This requirement is the missing element in our account\begin{changebar}; it can fix\end{changebar} its first weakness.\end{changebar}: [ii.] Actively elicit criticisms.
...
The justificatory task to address criticisms therefore appears infinite, and including it in rules \begin{changebar}and claiming that decision analysts should abide by these rules\end{changebar} might accordingly seem unrealistic.
...
\begin{changebar}The practical rules spelled out above echo \citet{lahtinen_why_2017}'s ``path approach'' to \ac{OR} activities, and their promotion of an ``ideal path'', which they describe as a ``path formed by well-justified choices''.
