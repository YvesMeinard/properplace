...
We have to find a way out of this conundrum without falling back on a variant of moral realism. \cite{meinard_what_2017} ventured a partial solution to the problem. This partial solution is based on two tenets. First, the idea that one will be able to capture a definitive list of criteria defining what is a good argument, what is the reasonable, what is acceptable, and so on, is illusory. One had better work incrementally, to improve step by step justifications, argumentations, etc. Second, justifiability is not a property of a recommendation formulated by a decision analyst, it is a property of the attitude of the analyst producing it.

This approach allowed to articulate a normative doctrine that can be translated into an answer to our question in the present article. This translation would state that, as decision scientists, our quest for justification should consist in:
\begin{itemize}
...
\noindent At this stage, skeptical readers will certainly think that our reasoning is a purely theoretical construct which will in all likelyhood face insuperable difficulties if one tries to implement it in practice. We indeed think that practival implementability is a crucial issue. Though we cannot have the ambition to definitely settle the implementability issue here, the present section will delve into more concrete considerations in order to sketch how our approach can overcome what we take to be the most difficult implementation challenge it faces. This challenge is the unavoidable rejoinder that the quest for justification will collapse on the pitfalls of disagreements and clashing orders of Jutification (Godard, Boltanski, Thevenot...).

For that purpose, let us take an example. The choice of this example among myriads of possible examples unavoidably involves some arbitrariness, but we will make a point to ensure that this arbitrariness will not undermine our conclusions. The example chosen are economic valuations of biodiversity. The choice of biodiversity as an object might seem extremely reductive. We claim that it is not, for two reasons. First, in practice, a very vast series of issues are actually gathered under the umbrella “biodiversity”, and accordingly focusing on economic valuations of biodiversity means focusing on applications of economic valuation methods to all sorts of environmental economic issues, ranging from the conservation of legally protected species to the quantification of ecosystem services (Kontoleon et al. 2007). Therefore, economic valuation of biodiversity actually is a vast subject. Second, in recent years, biodiversity as a valuation object provided the opportunity for researchers to introduce many methodological innovations, which can and possibly will be applied in the years to come to many other kinds of valuation objects (Bartkowski 2017). Therefore, economic valuations of biodiversity actually encompass a very large array of methods, perhaps more than any other valuation object.

The most prominently methods used in this domain are based on measurements of people's willingness to pay (WTP) (we will leave aside here the more anecdotal case of methods based on willingness to accept), as it can be elicited by surveys addressed at individual or revealed by these individuals' behavior on markets (Meinard and Grill 2011). The first case encompasses contingent valuation and choice experiment, while the second one mainly encompasses travel cost and hedonic pricing methods. A prominent alternative which has seen numerous empirical applications to biodiversity in recent years is deliberative valuation (Bartkowski 2017). This refers to methods based on choice-experiments or WTP questionnaires embedded in protocols of exchanges of information and discussions. These methods were originally motivated by critical discussions of the ontological assumptions underlying WTP-based valuation methods, and more recently Bartkowski and Lienhoop (2018) explored their positive philosophical underpinnings, referring mainly to Sen (2010). A third, much less developed method was introduced by Meinard et al. (2017). Based on theories of impartialization (Kolm 2005) protocols and on a reading of Rawls' philosophy, this method attempts to capture the impartial preference of citizens for the funding of nation-wide biodiversity conservation policies. As opposed to WTP-based valuation methods, an interesting feature of the literature on deliberative valuation and impartial preference measurement is that this literature explicitly explores the respective normative justifications of the methods. In order to compare these methods with standard valuation ones in terms of their respective credentials to partake in public reason, one needs a similar identification of normative justifications. A reconstruction of that kind was attempted by Meinard et al. (2016), who proposed a typology of standard valuation methods based on their respective possible normative justifications. Thanks to the elements articulated in the above publications, one can therefore identify a series of argumentative justifications that can be deployed to defend all three kinds of methods.

One might be tempted to conclude that, seen through the lenses of a quest for justification, all three methods survive the test. Or rather, that various people will in all likelyhood disagree on which justification is relevant. Such a scenario, where various groups of people adhere to different and largely irreconcialiable frameworks of justifications or so-called ``orders of justification''), is indeed considered to be a major phenomenon by many authors in environmental and social economics (Godard, Boltanski, Thevenot, Chateaureynaud...).

Though we take this criticism very seriously, we argue that it misses an important aspect of our approach. ThiThe fact that argumented justifications can be carved out for the different methods 


 simply means that the methods can partake in public reason, if the justifications at issue are acceptable. But how can we know if a justification is acceptable?
A natural but mistaken answer would consist in claiming that a justification qualifies as acceptable if and only if it turns out to be accepted in all the situations in which it can be applied. However, here again, how can one perform that kind of test? At best one can say whether applications of a given methods have so far been accepted, but this leaves aside all possible but non actual applications, and replaces acceptability by acceptance (falling in the trap that Habermas had earmarked in his criticism of Rawls). This insuperable problem suggests, in our view, that the question of whether methods can contribute to public reason is too general and abstract to be answered. The issue becomes addressable only if recast in the more concrete terms of the decision aiding processes (understood in the sense spelled out in Tsoukias 2007) in which they take part. Decision aiding processes are sets of continued interactions that can themselves be judged as to whether they qualify as contributions to public reason. This suggests that a more promising way to investigate our question is to ask whether specific usages of a given method partake in decision aiding processes where the acceptability of their very usage is put to the test.  Recasting the issue at the scale of particular decision aiding processes thereby allows to investigate our problem in a more concrete way. However, it does not magically lifts the need to distinguish acceptance from acceptability, which appears to be a glass ceiling preventing us from reaching a definitive conclusion. Indeed, we will argue that this limitation is enshrined in the very notion of public reason, normatively understood.

\section{Conclusion}
\noindent In this article, we have introduced a normative account of the role of decision science in a democracy. For that purpose, we have emphasized the failure of various strategies designed to allow decision scientists to eschew value-judgements, flight ``substance'' and remain perfectly neutral. Such attempts take the crude forms of a failed model of economics as pure science and a failed model of decision science as a transparent norm translater, and the subtler form of a model that endorses pure proceduralism to arrange a place for a purportedly value-neutral decision science upstream demcracy.

We have strived to demonstrate that all these models fail, which has led us to delve into an inquiry into a basic structure of moral reasoning, unveiling an important contrast between ``moral realism'' and the ``quest for justifications''. Because this inquiry drove us to recommend the latter, and because the latter can accordingly inform a normatively entrenched vision of decision sciences upstream democracy, we ended up recommending this model

 It is clearly part of our hope that this account of the normative stance of decision sciences will not come as a surprise for most decision scientists, and is rather liable to gather large support among them. However, though we accordingly expect that most decision scientist will agree with us at this stage, we surmise that most of them do not have clearly articulated ideas about the concrete implications of this basic normative stance. This is why the last section was devoted to develop such a concrete account, whose actual implementation might prove more disrupting for decision sciences practices than the cheer endorsement of our abstract normative account.
