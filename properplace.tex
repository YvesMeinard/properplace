...
\possessivecite{gass_ethical_2009} ``Oath of Prometheus'' similarly starts with a reference to \ac{DM}’s goals.\end{changebar}
...
We distinguish four attitudes ($A_O$, $A_E$, $A_I$ and $A_J$) which are reminiscent of \possessivecite{le_menestrel_ethics_2004} distinction between ``ethics outside'' (echoing our $A_O$), ``inside'' (echoing $A_E$) and ``beyond the model'' (of which $A_I$ and $A_J$ can be seen as variants). 
\commentOC{La phrase précédente est bien là, je trouve. J’effacerais la suivante. C’est une section de review, donc je ne crois pas qu’on doive expliquer en détail en quoi on va plus loin que l’existant. Ou alors, dire simplement que nos arguments et notre focus sont différents ?}
\commentYM{la phrase précédente me semble indispensable, car sinon on va nous reprocher de ne pas citer Le Menestrel. Mais une fois qu'on a mis cette phrase, il faut qu'on dise en 1 mot en quoi on fait autre chose de répéter Le Menestrel (ce qu'on ne fait pas). Ce que j'ai laissé de la phrase suivante revient selon moi à dire ``nos arguments et notre focus sont différents''. Est-ce que ça te va ?}
 However, our topic is broader, since we are interested here in the role that norms $N$ play in a decision support process as a whole, not only in models.%, and we introduce a distinction (between $A_I$ and $A_J$) without conterpart in \possessivecite{le_menestrel_ethics_2004} typology.
\end{changebar}
...
\citet[p. 180]{morgenstern_reflections_1979} can, under some interpretation of his text, be considered to be a proponent of this attitude, for example when he writes that 
“[expected utility] theory, as formulated by the von Neumann-Morgenstern axioms, is normative in the sense that the theory is ‘absolutely convincing’”.
\commentYM{je n'ai rien contre remettre Vénus ici, et je suis d'accord que l'argument du reviewer est bidon. J'enlève juste le ``is normative'' de la citation, car ce terme est une boîte de Pandore}
\commentOCf{C’est dommage de tronquer ainsi sa phrase, on pourrait nous reprocher de tenter de simplifier son propos. Surtout que la citation devient tellement courte qu’on peut se demander à quoi elle sert. Mais maintenant que j’y re-réfléchis, je me dis que c’est peut-être un peu osé de notre part de lui faire adopter cette attitude. Son texte est trop obscur pour qu’on sache s’il est plutôt du côté de $A_O$, $A_E$, ou autre chose encore. Laissons tomber. (Réfléchir à Brans ?)}
...
\subsubsection{\texorpdfstring{$A_E$}{AE}: choosing \protectforpdf{N} that \aclp{DM} “would endorse if they could understand”}
...
\begin{changebar}
This attitude is illustrated by \possessivecite{brans_ethics_2002} call for ``independent ethical experts'' to take charge of the ``ethical pole'' of decision support.% might be seen as a champion of $A_E$, at least in a plausible interpretation of his texts. He writes that: 
\end{changebar}
...
And because $R$ are necessarily anchored in norms $N$, this intelligibility requirement, when applied to the elements of the decision support process embodying $N$, unavoidably includes a requirement that the \ac{DM} \emph{endorses} $N$. 
...
\begin{changebar}
\subsubsection{\texorpdfstring{$A_J$}{AJ}: informally justifying \protectforpdf{N}}
...
In some contexts, one might call any argument, however spurious or ill-conceived, a ``justification''. \begin{changebar}This is illustrated by \citet{jackson_towards_1999}, who spells out in detail the resources that can be used to articulate justification, but does not explore how to ensure that an argument based on his guidelines qualifies as a justification.
...
As is stands, $A_J$ hence appears unsatisfactory because it takes for granted that the notion of justification is transparent and unproblematic, whereas on due reflexion this notion appears ambiguous.
\end{changebar}
...
This \begin{changebar}approach hence elaborates on the idea to produce justification, anchors this idea in a reference to acceptability, but strives to eschew specifying criteria to distinguish acceptable justifications from non-acceptable ones.\end{changebar}
...
by introducing a “weak” transcendental deduction of the tenets of “discourse ethics” from communicative action \cite{habermas_moralbewustsein_1983}. 
...
The role that norms play is therefore clearly circumscribed in Habermas's framework, and it concerns one kind of validity claims among others, and he would probably not claim that recommendations based on such norms can be justified. 
...
\begin{changebar}
The norms specifically constituting Habermas's discourse ethic are hence anchored in his philosophy of society, which endows them with a specific justification refering to Habermas's theory of communicative action.
...
In \begin{changebar}this section, our aim is to elaborate on the idea outlined by $A_J$ and (according to our argument) unsatisfactorily elaborated by Rawls and Habermas, that a convenient higher level norm $\adv$ should embody a justification requirement.\end{changebar}
...
\commentOCf{Tu avais commenté cette section, est-ce pour des raisons de place ? Si on a la place, mériterait-elle une restauration ? Nommer explicitement cette notion de \og{}primacy of practice\fg{} me semble utile.}
\commentYM{Le reviewer 1 a suggéré d'enlever car il ne voit pas à quoi sert cette notion. Indépendamment, dans le cadre d'un speech que j'ai fait sur ce paper le mois dernier, je me suis rendu compte que son utilité n'était pas si évidente que ça, et qu'il m'était plus facile d'introduire la précédure en ne parlant que des 3 contraintes. Mais si tu trouves qu'elle apporte vraiment quelque chose ci-dessous, je ne suis pas fermé.} 
But how can one know that a justification is acceptable in this sense? 
We have seen above that \cite{estlund_democratic_2009}’s solution would consist, for the philosopher, in making bold claims about the truth of the criteria used to sort out what is acceptable and what is not. 
But a major problem with such an approach is that it contradicts our above requirements of \emph{incrementalism} and \emph{anchorage in real-life acceptability}. 
...
The inclusion of other people can by itself bring in new insights or information.\end{changebar}
...
We now need a third clause to allow \cref{it:argue,it:criticize} to fully embody \commentOCf{Envisager de restaurer ici : “primacy of practice”, by putting justifications…} the requirement to put justifications to the test in real-life, instead of confining them to theoretical criteria: Actively defend our recommendations against all criticisms (\ref{it:defend}).
...
\commentOCf{On ne peut pas dire ça sans la suite (commentée), il me semble (bien que c’était là lors de la version soumise). Et la phrase ne semble pas à sa place ici, car ce propos semble général, non associé à l’exemple. Je changerais légèrement pour instancier le propos à l’exemple, je restaurerais le commentaire, et je dirais qu’il illustre \cref{it\string:provisional}.}
\commentYM{j'avais enlevé la discussion sur le DM buté, suite (me semble-t-il) à un commentaire de ta part dans cette direction. Je ne vois pas comment on peut en réinstaurer des parties sans réinstaurer tout}
...
However, \citeauthor{funtowicz_science_1993} neglect the difficulties involved in implementing [\ref{it:argue}-\ref{it:criticize}], which prompted us to introduce [\ref{it:defend}-\ref{it:provisional}] and, as opposed to \citeauthor{funtowicz_science_1993}, we argue that the procedure [\ref{it:argue}-\ref{it:provisional}] should be deployed in all decision support processes, 
...
Besides, as emphasized above, our work in this article is focused on relatively simple situations where the recommendations elaborated by the analyst are offered to a single, well-identified \ac{DM}.
...
\section{References}\commentYM{je n'ai pas compris tout ce que tu as fait sur la biblio, mais je pense qu'on ne s'est pas compris, car je pense qu'il faut mettre les doi}
