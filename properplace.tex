...
\documentclass[preprint, french, english, 11pt, authoryear]{elsarticle}
\usepackage{lineno}
...
\address[ld]{Université Paris-Dauphine, PSL Research University, CNRS, UMR [7243], LAMSADE, 75016 PARIS, FRANCE}
...
As emphasized in particular in the literature on ethical dimensions of \ac{OR} practice, such recommendations are unavoidably conditioned by norms or normative conceptions.
...
We then introduce and recommend a series of practical rules that decision analysts should abide by, on the basis of which, in a given decision situation, a decision analyst can decide, together with the \ac{DM}, whether a given norm underlying a given recommendation can be adopted.
...
Here, we understand the content of this $N$ in a very broad sense, and take it to refer to all the elements that must be associated with factual statements in order to derive recommendations. 
...
In addition to being pervasive, the complexity of value judgements and their contexts of application create practical dilemmas which cannot be easily solved using general ethical guidelines or codes of conduct \citep{taket_undercover_1994}, and call for interdisciplinary collaboration involving basic research in philosophy, applied ethics and \ac{OR} \citep{picavet_opportunities_2009}. Accordingly, because “\ac{OR}  
is a human activity in which \ac{OR} 
workers engage with other humans to improve human-activity systems” \citep[p. 291]{ormerod_operational_2013}, which unavoidably raises “explicit or implicit moral issues” \citep[p. 132]{diekmann_moral_2013}, there is a large consensus, in both \ac{OR} 
...
\possessivecite[p. 195]{brans_or_2002} “Oath of Prometheus” likewise includes a reference to \acp{DM}’ goals: 
“As a consultant or an analyst, I commit myself (...) \emph{to assisting [decision makers] to meet their goals} within the limits of sustainable development” (emphasis ours).
Although there are debates among \ac{OR} researchers and practitioners on the extent to which analysts should support the values and objectives of \acp{DM} \citep{brans_ethics_2007}, even those authors who are the most prone to accept that analysts can question and override \acp{DM}’ values and objectives acknowledge that \acp{DM} have a \emph{ceteris paribus} legitimacy to express their values and objectives and to expect them to be respected (this is the case, in particular, of contributions anchored in critical theory \citep{jackson_towards_1984, mingers_ethics_2011}).
A weaker form of neutrality, encapsulated in the requirement to refrain from being \emph{dogmatic, paternalist, proselyte, or authoritarian}, by imposing $N$ we favor in our interaction with \acp{DM}, is hence largely accepted. However, because “most contributions to the ‘ethics’ of \ac{OR}, well-taken as they are in drawing our attention to the value content of all \ac{OR} practice, tell us (...) little about how to handle ethical conflicts” \citep[p. 294]{ormerod_operational_2013},
...
Our approach for this review is interpretative: we infer a proposal for a classification of attitudes with respect to $N$ based on our knowledge of the literature in economics, philosophy and \ac{OR}. This methodological choice is motivated by the fact that interpretative approaches allow the articulation of refined judgments on the meaning of practices such as \ac{OR} (echoing the importance of narrative analysis to understand such practices \citep{white_death_1994}).
That said, like any hermeneutic claim, the one we are about to articulate is tentative.
A more systematic methodology, complementing our qualitative analysis with a quantitative review of the literature, would certainly provide additional, important insights. 
...
Most contributions address it peripherally, in the sense that their main focus is not to theorize their attitude towards $N$. Others take advantage of an existing philosophical framework delineating an attitude towards $N$, but do not discuss this attitude for itself.
...
Attitude $A_E$ consists, for the decision analyst, in endorsing a higher-level norm $\mathscr{N}_E$ claiming that expert discussions about norms enable them to make decisions about norms on behalf of \acp{DM}, a questionable moral stance that \citet[p. 40]{estlund_democratic_2009} calls the “expert/boss fallacy”. 
...
This is the attitude of decision analysts who reject $A_E$’s idea that decision scientists can decide on $N$ on behalf of \acp{DM}, and therefore make a point of informally discussing the meaning of $N$ with \acp{DM} to verify that $N$ appears endorsable to the \acp{DM}.
...
Although they do not explicitly tackle our problem, many authors in the literature, especially in ethics and methodology of \ac{OR}, endorse a fourth attitude that can seem to offer a solution to our problem. This candidate solution consists in advocating that analysts should \emph{justify} the decision support they provide. If applied to our reflexion on $N$, this idea suggests that analysts should justify the $N$ from which their recommendations derive (or, equivalently, should produce assertions concerning these $N$ which are all “warranted”).
For example, \citet[p. 75]{lahtinen_why_2017} advocate \ac{OR} practices following an “ideal path”, “formed by well-\emph{justified} choices”, which presumably include choices concerning $N$, such as choices of goals or principles.
Similarly, \possessivecite[p. 135]{diekmann_moral_2013} defense of “transparency”, in his exploration of the principles that should underlie modeling, is explicitly anchored in a justification requirement, defining transparency in terms of “open communication” and “clarification”. His presentation of “integrity” explicitly refers to justification.
In the context of reviewing the morality of \ac{OR} practice, \citet{ormerod_operational_2013} ask how analysts can justify their \emph{ethical} positions.
 \possessivecite{beauchamp_principles_2009} criteria to balance principles when they conflict (in matters of biomedical ethics) also explicitly refer to justification (in this case: justification of the $N$ used to balance principles).
\possessivecite{white_death_1994} call to demystify expertise enjoin experts to justify their interventions, which, given the authors’ insistence on the post-modern view that expertise is pervasively value-laden, involves a justification of $N$.
Although he does not focus on values or norms, \citet[p. 21]{jackson_towards_1999} concurs on the importance of justification and spells out “guidelines” that justifications “must” follow to “claim to have used a management science methodology according to a particular rationale”.
Lastly, \citet{ormerod_justifying_2010} even argues that a willingness to justify, and in particular to justify \emph{values} underlying OR, is a necessary condition for decision scientists to present themselves as \emph{scientists}.
...
This was supposed to be a complete “flight from substance” \citep[p. 85]{estlund_democratic_2009}, in the sense that this account was supposed to eschew all forms of value judgments.
...
Besides, as \citet[p. 146]{cronin_issues_2014} point out, “Often participants may not even be aware of the preconceptions, or assumptions, that they are taking for granted” and interactions with \acp{DM} 
“are inevitably subject to various constraints some of which arise pre-consciously as a result of historically acquired competences and predispositions to operate in particular ways” \citep[p. 1075]{brocklesby_ethics_2009}.
...
(this problem is not addressed by \cite{meinard_what_2017}, and is also left aside by \cite{mingers_ethics_2011}, who mentions this issue without exploring it when acknowledging a limit to his own framework under the heading “Engagement and inclusion”, p. 122). 
...
The practical rules spelled out above echo \possessivecite[p. 75]{lahtinen_why_2017} “path” approach to \ac{OR} activities, and their promotion of an “ideal path”, which they describe as a “path formed by well-justified choices”.
Our own practical rules aim at spelling out in more concrete terms the requirements that \citet[p. 75]{lahtinen_why_2017} refer to using terms such as “well-justified”, “essential characteristics”, “reflective”.
Similarly, our \cref{it:argue} echoes \possessivecite[p. 739]{funtowicz_science_1993} claim that in post-normal science “values are not presupposed but made explicit”, and our \cref{it:criticize} is reminiscent of \citeauthor{funtowicz_science_1993}’s “extended peer community”.
...
A similar relation exists with \citet[p. 121]{mingers_ethics_2011}, who delineates applications of Habermas’s discourse ethics to \ac{OR} practice, but uses the term “justifiable” without defining it and advocates that what is important is “that every effort is made to involve and engage all the relevant stakeholders in a genuine and continuing dialogue” without specifying how to ensure that the dialog is “genuine”.
Although, at this stage, we do not explore in concrete behavioral terms the interaction between analysts and \acp{DM}, our approach also interestingly echoes \citet[p. 628]{hamalainen_importance_2013} who emphasize, in their review of the importance of taking account of behavioral aspects in \ac{OR}, 
...
In particular, our approach to “justification” is anchored in an analysis of the usages of the term. Besides, we propose to unfold the requirement to produce justification as a practical procedure and, even more fundamentally, 
...
We thereby leave aside difficulties associated with group decisions \citep{jackson_towards_1984}, stakeholder identification \citep{wang_systemic_2015, gregory_stakeholder_2019} and with boundary judgments involving the integration of several individuals in a collective \ac{DM} \citep{midgley_systemic_2000}
