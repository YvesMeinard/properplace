...
\title{Justification and decision sciences}\commentYM{si notre manière de parler des sciences de la décision te mettais mal à l'aise, j'imagine que le titre aussi ! changer pour ``justification and applied decision analysis'' ?}
...
%By talking about “normative status”, we mean that we want to clarify the extent to which these practices are normative, and investigate how (or whether) this normative status can remain compatible with the ambition, for decision sciences, to deserve to be called ``sciences''. \commentYM{voir si une autre formulation ne serait pas préférable. Le pb est que nous n'explorons pas le concept de science.}
...
%This picture of the practice of decision analysis is simplistic.In fact, the analysts' conclusion in such a situation is typically not a self-standing $R$, but rather contains recommendations $R$ conditioned by norms, normative conceptions or value judgments $N$. \commentOC{Je dirais plus simplement : In fact, the analysts' conclusion in such a situation is typically…} \commentYM{je ne comprends pas bien la modif que tu préconises ici, je ne vois pas quelle partie complexe tu veux remplacer par du ``plus simple''} In a broader setting, this anchorage of science-based recommandations in normative conceptions was famously emphasized by the literature on ``post-normal science'' \citep{funtowicz_science_1993}.
...
\end{addmargin}
\commentYM{je ne garderais que la première phrase de la citation ci-dessus}
