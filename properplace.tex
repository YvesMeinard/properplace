...
\begin{changebar}
Our approach is to start from naïve attitudes (developed in the first two sub-sections), recalling the reasons to consider them unsatisfactory, and progress towards more elaborated ones that are actually promoted by researchers in the literature. 
We indicate here as well why we consider them as not fully satisfactory solutions to our problem.
\commentYM{je ne comprends pas tout à ta proposition ici.
Tu enlèves ``Our approach to establish this typology is interpretative: we infer a proposal for a classification based on our knowledge of the literature in economics, philosophy and \ac{OR}.
Like any hermeneutic claim, our proposed typology is tentative.''
Alors que cela me semble indispensable, et indépendant de ce que tu cherches à traiter dans ta modification.
Par ailleurs, tu opères un contraste entre deux premières sous-sections ``naïves'' et les deux suivantes qui seraient ``actual''.
Pour moi un tel contraste n'existe pas : les gens naïfs, ça existe ``actually''.
Ao est, incontestablement selon moi une approche naïve acceptée (plus ou moins consciemmment, par bcp d'économistes.
Et je ne vois aucune raison de ne pas citer Brans comme un exemple de AE.
Cependant, je suis d'accord avec ton argument selon lequel on risque de nous accuser de mettre le feu à des hommes de paille, ce qui appelle, selon moi, une modification plus limitée du texte originel, consistant simplement à dire ``ok, nos attitudes sont stylisées, du genre :
...
Like any hermeneutic claim, our proposed typology is tentative, and the attitudes that we describe are stylized, ideal-typical representations of the more complex stances of real-life analysts and authors.
}
